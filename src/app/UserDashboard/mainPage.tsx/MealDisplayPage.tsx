...
const MealDisplay: React.FC<MealInterface> = ({ email }) => {
  ...
  const getUserFoodLogDates = async () => {
    ...
    try {
      ...
      const fetchedDates = sortDatesAscending(
        userDocSnap.docs.map((doc) => doc.id)
      ); // getFormattedDateTime() values
  ...
  const getFoodDocs = async (selectedDate: string) => {
    ...
    try {
      ...
      const fetchedFoods = userDocSnap.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
          ...
          <table className="w-full border-collapse bg-gray-900 rounded-lg overflow-hidden shadow-lg">
            <thead>
              <tr className="bg-blue-900">
                ...
                <th className="p-4 text-right w-1/4">
                  Daily: {calculateTotalCalories()} kcal
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                <React.Fragment key={mealType}>
                  ...
                  >
                    ...
                    <td className="p-3 text-right">
                      {calculateMealCalories(foods)} kcal
                    </td>
                    <td className="p-3 text-right">
                      {Math.round(
