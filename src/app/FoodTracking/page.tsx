...
export default function FoodTracker() {
  ...
  const calculateCalories = () => {
    const totalCalories = foodNutrients.reduce(
  ...
  const calculateProtein = () => {
    const totalProtein = foodNutrients.reduce(
      (acc, item) => acc + item.macros.protein,
      0
    );
  ...
  const calculateCarbs = () => {
    const totalCarbs = foodNutrients.reduce(
      (acc, item) => acc + item.macros.carbohydrates,
      0
    );
    setCarbs(totalCarbs);
  };
  ...
  const calculateFats = () => {
    const totalFats = foodNutrients.reduce(
      (acc, item) => acc + item.macros.fat,
      0
    );
    setFats(totalFats);
  };
      ...
      <div className="max-w-6xl mx-auto px-6 relative flex-grow pt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-[#1a1725] rounded-xl p-6">
            ...
            <div className="text-2xl font-bold">
              {proteins}/{detectedFoods.length * 100}g
            </div>
                ...
                style={{
                  width: `${
                    detectedFoods.length > 0
                      ? (proteins / (detectedFoods.length * 100)) * 100
                      : 0
                  }%`,
                }}
              ></div>
          ...
          <div className="bg-[#1a1725] rounded-xl p-6">
            ...
            <div className="text-2xl font-bold">
              {carbs}/{detectedFoods.length * 100}g
            </div>
            <div className="h-1.5 bg-white/10 rounded-full mt-3">
              <div
                className="h-full bg-purple-500 rounded-full"
                style={{
                  width: `${
                    detectedFoods.length > 0
                      ? (carbs / (detectedFoods.length * 100)) * 100
                      : 0
                  }%`,
                }}
              ></div>
          ...
          <div className="bg-[#1a1725] rounded-xl p-6">
            ...
            <div className="text-2xl font-bold">
              {fats}/{detectedFoods.length * 100}g
            </div>
            <div className="h-1.5 bg-white/10 rounded-full mt-3">
              <div
                className="h-full bg-purple-500 rounded-full"
                style={{
                  width: `${
                    detectedFoods.length > 0
                      ? (fats / (detectedFoods.length * 100)) * 100
                      : 0
                  }%`,
                }}
              ></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#1a1725] rounded-xl p-6">
              ...
              {detectedFoods.length > 0 ? (
                <div className="grid grid-cols-[75%_25%] w-full ">
                  {/* Table Header */}
                  <div className="p-2 font-bold border-b border-gray-300">
                    Food
                  </div>
                  <div className="p-2 font-bold border-b border-gray-300">
                    Quantity
                  ...
                  {detectedFoods.map((food, index) => (
                    <React.Fragment key={index}>
                      ...
                      >
                        <span className="underline">{food.label}</span>
                      </div>
            ...
            {success && (
              <div className="rounded p-6 bg-green-600 text-white">
                Food Added Sucessfully
              </div>
            )}
            {errorMessage && (
              <div className="rounded p-6 bg-red-600 text-white">
                {errorMessage}
              </div>
            )}
            ...
            <div className="mt-6">
              ...
              <div className="mt-3 bg-green-900/20 border-l-2 border-green-600 p-4 rounded-lg text-green-300">
                Try adding your meal by uploading a photo or using your camera.
                I'll help identify the food and estimate calories.
              </div>
            </div>
          </div>
        </div>
